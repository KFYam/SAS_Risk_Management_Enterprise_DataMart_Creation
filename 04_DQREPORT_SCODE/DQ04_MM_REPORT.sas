...
/* ************************************************************************************************* */
%macro gen_MM_DQReport(stdte=, rmp=);
	...
	proc format;
		...
		4 -< 5 = "4. Already defaulted at the beginning of the reporting period (final rating = 8)"
	...
	data work.datalist;
		...
		_index = put(strip(memname),$mm_ds.);
	run;
	data dummy;
		length label $255;
	...
	/* *************************************************************************************************** */
	options missing='';
	ods tagsets.ExcelXP file="&dir_rptdq./DQ_Data_MM_&stdte..xls" style=minimal;
	* Exclusion Summary *;
	...
	* Default Data *;
	%if %sysfunc(exist(work.default_MM_&stdteb.)) %then %do;
		ods tagsets.ExcelXP options(sheet_name='Default');
		proc print data=work.default_MM_&stdteb. noobs label; run;
	%end;
	* Report Info *;
	...
	options missing=.;
%mend;
%gen_MM_DQReport(stdte=&st_BackMth.,rmp=RMP2BIOMM);
%gen_MM_DQReport(stdte=&st_RptMth.,	rmp=RMP2BIOMM);
