...
/* ************************************************************************************************* */
%macro gen_PF_DQReport(stdte=, rmp=);
	...
	data work.datalist;
		...
		_index = put(strip(memname),$PF_ds.);
	run;
	data dummy;
		length label $255;
	...
	/* *************************************************************************************************** */
	options missing='';
	ods tagsets.ExcelXP file="&dir_rptdq./DQ_Data_PF_&stdte..xls" style=minimal;
	* Exclusion Summary *;
	ods tagsets.ExcelXP options(sheet_name='src_Excluded');
		...
		proc freq data= mart.&rmp._key_&stdte. order=formatted;;
			...
			format sasflag_dq_excluded PF_excl.;
		run;
		*/
	...
	* Default Data *;
	%if %sysfunc(exist(work.default_PF_&stdteb.)) %then %do;
	ods tagsets.ExcelXP options(sheet_name='Default');
		proc print data=work.default_PF_&stdteb. noobs label; run;
	%end;
	* Report Info *;
	...
	options missing=.;
%mend;
%gen_PF_DQReport(stdte=&st_BackMth., rmp=RMP2BIOPF);
%gen_PF_DQReport(stdte=&st_RptMth.,	 rmp=RMP2BIOPF);
