...
/* ************************************************************************************************* */
%macro gen_REDI_DQReport(stdte=, rmp=);
	...
	data work.datalist;
		...
		_index = put(strip(memname),$REDI_ds.);
	run;
	data dummy;
		length label $255;
	...
	%if %sysfunc(exist(mart.&rmp._&stdteb.)) %then %do;
		data work.default_REDI_&stdteb.;
	...
	/* *************************************************************************************************** */
	options missing='';
	ods tagsets.ExcelXP file="&dir_rptdq./DQ_Data_REDI_&stdte..xls" style=minimal;
	* Exclusion Summary *;
	...
	proc freq data= mart.&rmp._key_&stdte. order=formatted;;
		...
		format sasflag_dq_excluded REDI_excl.;
	run;
	*/
	...
	* Default Data *;
	%if %sysfunc(exist(work.default_REDI_&stdteb.)) %then %do;
	ods tagsets.ExcelXP options(sheet_name='Default');
		proc print data=work.default_REDI_&stdteb. noobs label; run;
	%end;
	* Report Info *;
	...
	options missing=.;
%mend;
%gen_REDI_DQReport(stdte=&st_BackMth., rmp=RMP2BIOREDI);
%gen_REDI_DQReport(stdte=&st_RptMth.,  rmp=RMP2BIOREDI);
