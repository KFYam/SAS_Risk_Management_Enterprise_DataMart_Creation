...
/* ************************************************************************************************* */
%macro gen_OF_DQReport(stdte=, rmp=);
	...
	data work.datalist;
		...
		_index = put(strip(memname),$OF_ds.);
	run;
	data dummy;
		length label $255;
	...
	/* *************************************************************************************************** */
	options missing='';
	ods tagsets.ExcelXP file="&dir_rptdq./DQ_Data_OF_&stdte..xls" style=minimal;
	* Exclusion Summary *;
	ods tagsets.ExcelXP options(sheet_name='src_Excluded');
		...
		proc freq data= mart.&rmp._key_&stdte. order=formatted;;
			...
			format sasflag_dq_excluded OF_excl.;
		run;
		*/
		proc means data=mart.&rmp._key_&stdte. n completetypes nonobs maxdec=0;
	...
	* Default Data *;
	%if %sysfunc(exist(work.default_OF_&stdteb.)) %then %do;
	ods tagsets.ExcelXP options(sheet_name='Default');
		proc print data=work.default_OF_&stdteb. noobs label; run;
	%end;
	* External Rating *;
	ods tagsets.ExcelXP options(sheet_name='ExRating');
		proc print data=work.exrating_&stdte. noobs label; run;
	...
	options missing=.;
%mend;
%gen_OF_DQReport(stdte=&st_BackMth., rmp=RMP2BIO10);
%gen_OF_DQReport(stdte=&st_RptMth.,	 rmp=RMP2BIO10);
