...
/* ************************************************************************************************* */
%macro gen_NBFIINS_DQReport(stdte=, rmp=);
	...
	proc format;
		...
		2 -< 3 = "1. Rating expired at the report date (next_review_date <= &stYMD.)"
		4 -< 5 = "2. Already defaulted at the beginning of the reporting period (final rating = 8)"
		5 -< 6 = "3. Incomplete Financials"
		;
		value $NBFIINS_ds
		"&rmp.A_&stdte." = "A"
		"&rmp.B_&stdte." = "B";
	...
	data work.datalist;
		...
		_index = put(strip(memname),$NBFIINS_ds.);
	run;
	data dummy;
		length label $255;
	...
	%if %sysfunc(exist(mart.&rmp._&stdteb.)) %then %do;
		data work.default_NBFIINS_&stdteb.;
	...
	/* *************************************************************************************************** */
	options missing='';
	ods tagsets.ExcelXP file="&dir_rptdq./DQ_Data_NBFIINS_&stdte..xls" style=minimal;
	* Exclusion Summary *;
	...
	proc freq data= mart.&rmp._key_&stdte. order=formatted;;
		...
		format sasflag_dq_excluded NBFIINS_excl.;
	run;
	*/
	...
	* Default Data *;
	%if %sysfunc(exist(work.default_NBFIINS_&stdteb.)) %then %do;
	ods tagsets.ExcelXP options(sheet_name='Default');
		proc print data=work.default_NBFIINS_&stdteb. noobs label; run;
	%end;
	* Report Info *;
	...
	options missing=.;
%mend;
%gen_NBFIINS_DQReport(stdte=&st_BackMth.,	rmp=RMP2BIONBFIINS);
%gen_NBFIINS_DQReport(stdte=&st_RptMth.,	rmp=RMP2BIONBFIINS);
