...
/* ************************************************************************************************* */
%macro gen_REIMS_DQReport(stdte=, rmp=);
	...
	proc sort data=raw_rmp.&rmp._&stdte. out=tmp_keytbl(keep=n_raid n_archive_id sasflag_dq_excluded) nodupkey; 
	...
	data work.datalist;
		...
		_index = put(strip(memname),$REIMS_ds.);
	run;
	data dummy;
		length label $255;
	...
	%if %sysfunc(exist(mart.&rmp._&stdteb.)) %then %do;
		data work.default_REIMS_&stdteb.;
	...
	/* *************************************************************************************************** */
	options missing='';
	ods tagsets.ExcelXP file="&dir_rptdq./DQ_Data_REIMS_&stdte..xls" style=minimal;
	* Exclusion Summary *;
	...
	proc freq data=tmp_keytbl order=formatted;;
		table sasflag_dq_excluded/missing nopercent nocum; 
		format sasflag_dq_excluded REIMS_excl.;
	run;
	*/
	...
	* Default Data *;
	%if %sysfunc(exist(work.default_REIMS_&stdteb.)) %then %do;
	ods tagsets.ExcelXP options(sheet_name='Default');
		proc print data=work.default_REIMS_&stdteb. noobs label; run;
	%end;
	* Report Info *;
	...
	options missing=.;
%mend;
%gen_REIMS_DQReport(stdte=&st_BackMth., rmp=RMP2BIOREIMS);
%gen_REIMS_DQReport(stdte=&st_RptMth.,	rmp=RMP2BIOREIMS);
