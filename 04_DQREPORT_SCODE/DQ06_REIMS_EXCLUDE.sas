...
/* For REIMS PD */
/* ************************************************************************************* */
%macro gen_REIMS_Exclusion(stdte=, rmp=);
	...
	data tmp_rmp;
		...
		if missing(sasflag_dq_excluded) and not missing(car_model) and strip(car_model) ne "RE Investor - Medium & Small Size Model" then sasflag_dq_excluded = .S;
		...
		/* Funding vehicle is not applicable for REIMS */
		...
		/* Incompleted FS indicator exists in source file, but CMM does not include this checking; have to double check with them */
...
%mend;
%gen_REIMS_Exclusion(stdte=&st_BackMth.,	rmp=RMP2BIOREIMS);
%gen_REIMS_Exclusion(stdte=&st_RptMth.,		rmp=RMP2BIOREIMS);
