...
/* ************************************************************************************************* */
%macro gen_LC_DQReport(stdte=, rmp=);
	...
	data work.datalist;
		...
		_index = put(strip(memname),$lc_ds.);
	run;
	data dummy;
		length label $255;
	...
	/* *************************************************************************************************** */
	options missing='';
	ods tagsets.ExcelXP file="&dir_rptdq./DQ_Data_LC_&stdte..xls" style=minimal;
	* Exclusion Summary *;
	ods tagsets.ExcelXP options(sheet_name='src_Excluded');
		...
		proc freq data= mart.&rmp._key_&stdte. order=formatted;;
			...
			format sasflag_dq_excluded lc_excl.;
		run;
		*/
	...
	* Default Data *;
	%if %sysfunc(exist(work.default_LC_&stdteb.)) %then %do;
	ods tagsets.ExcelXP options(sheet_name='Default');
		proc print data=work.default_LC_&stdteb. noobs label; run;
	%end;
	* Report Info *;
	...
	options missing=.;
%mend;
%gen_LC_DQReport(stdte=&st_BackMth.,rmp=RMP2BIOLCM);
%gen_LC_DQReport(stdte=&st_RptMth.,	rmp=RMP2BIOLCM);
